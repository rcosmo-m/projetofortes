...
window.initializeMaps = () => {
  window.initEmpresaMap =
    window.initEmpresaMap ||
    (() => {}) // Declare initEmpresaMap variable // Declare initEmpresaMap variable
  window.initInstituicaoMap =
    window.initInstituicaoMap ||
    (() => {}) // Declare initInstituicaoMap variable // Declare initInstituicaoMap variable
  initEmpresaMap()
  initInstituicaoMap()
}
...
function initEmpresaMap() {
  ...
  map.addListener("click", (event) => {
    if (document.getElementById("empresa-pontos").classList.contains("active")) {
...
function setupMapControls(map, mapElement) {
  const controlsContainer = mapElement.parentElement.querySelector(".map-controls")
  ...
  if (controlsContainer) {
    ...
    const locationBtn = controlsContainer.querySelector('[title="Minha Localização"]')
    if (locationBtn) {
...
function showAddPointModal(location, map) {
  ...
  if (confirmAdd) {
    ...
    form.addEventListener("submit", (e) => {
      ...
      const wasteTypes = []
      if (document.getElementById("waste-paper").checked) wasteTypes.push("Papel")
      if (document.getElementById("waste-plastic").checked) wasteTypes.push("Plástico")
      if (document.getElementById("waste-metal").checked) wasteTypes.push("Metal")
      if (document.getElementById("waste-glass").checked) wasteTypes.push("Vidro")
      if (document.getElementById("waste-electronics").checked) wasteTypes.push("Eletrônicos")
...
window.agendarColeta = (pontoNome) => {
  showToast(`Agendamento para ${pontoNome} será implementada em breve!`, "info")
}
